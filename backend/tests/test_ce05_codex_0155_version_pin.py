...
@pytest.mark.asyncio
async def test_codex_0155_alpha16_4_pin_rejects_previous_alpha16_3_before_capability_auth(
    ...
    async def fake_exec(*argv: str, **_kwargs: Any) -> _VersionProcess:
        calls.append(argv)
        if argv[1:] == ("--version",):
            return _VersionProcess(stdout=b"codex-cli 0.155.0-alpha.16.3")
        pytest.fail(f"unexpected subprocess after version mismatch: {argv!r}")

    assert CODEX_CLI_APPROVED_VERSION == "codex-cli 0.155.0-alpha.16.4"
