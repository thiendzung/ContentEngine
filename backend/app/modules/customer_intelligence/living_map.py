...
async def ensure_customer_insight_need_link(
    ...
    *,
    ...
    if not actor:
        raise CustomerMapError("customer_map_insight_need_actor_required")
    if not rationale:
        raise CustomerMapError("customer_map_insight_need_reason_required")
...
def compare_customer_map_snapshots(
    ...
    for need_id in sorted(set(current_needs) & set(previous_needs)):
        events.extend(
            _need_evidence_events(
                need_id=need_id,
                previous=previous_needs[need_id],
                current=current_needs[need_id],
            )
        )
