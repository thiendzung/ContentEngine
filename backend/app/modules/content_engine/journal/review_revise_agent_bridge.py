...
def render_review_revise_prompt(
    ...
    *,
    ...
    return (
        ...
        "relation is supports or qualifies count as support for those claims. "
        "relation=context_only and relation=contradicts never count as supportive evidence. "
        "If a claim lacks exact support, remove the factual claim or rewrite the passage as "
        "bounded reader guidance. Do not replace an unsupported fact with a broad universal "
        "or epistemic claim that would itself require external proof. Prefer direct reader "
        "actions such as check, confirm, verify, compare or ask when the available material "
        "only supports guidance. Preserve the accepted section IDs/order and exact "
        "evidence/originality ref arrays; do not add or substitute refs.\n\n"
        "TITLE SUPPORT CONTRACT:\n"
        "The Writer schema has no evidence-ref or originality-ref fields for title, and "
        "Assertion Audit therefore gives the title zero allowed support refs. The title must "
        "stay non-assertive and evidence-neutral: do not state or imply geographic origin, "
        "local making, provenance, authenticity, artist identity or intent, material facts, "
        "quality, market facts or other externally verifiable propositions without an allowed "
        "support binding. If the source title contains such a proposition, rewrite it as "
        "reader guidance or a neutral topic label while preserving the article's approved "
        ...
        "closing_markdown must not contain factual, brand, artist-intent, visual-observation "
        "or live-information assertions. If a supported factual point already belongs in an "
        "existing ref-bound section, keep it there and remove it from the closing. Rewrite the "
        "closing as non-assertive reader guidance. Do not invent new closing ref fields and do "
        "not move or substitute the accepted support-ref arrays.\n\n"
        f"RECIPE_JSON:\n{recipe_json}\n\n"
