...
class ContentOpportunity(TimestampMixin, Base):
    ...
    existing_content_refs_json: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
...
class ContentOpportunitySignal(Base):
    ...
    content_opportunity_id: Mapped[UUID] = mapped_column(
        ForeignKey("content_opportunities.id", ondelete="CASCADE"), primary_key=True
    )
    signal_id: Mapped[UUID] = mapped_column(
        ForeignKey("signals.id", ondelete="CASCADE"), primary_key=True
    )
...
class ContentExperiment(TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("hypothesis_version > 0", name="ck_content_experiment_hypothesis_version"),
    )
...
class ContentCase(TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint("content_type in ('journal','artwork')", name="ck_content_cases_type"),
...
class ContentItem(TimestampMixin, Base):
    ...
    project_id: Mapped[UUID] = mapped_column(ForeignKey("projects.id"), nullable=False)
    content_case_id: Mapped[UUID] = mapped_column(ForeignKey("content_cases.id"), nullable=False)
    locale_variant_id: Mapped[UUID] = mapped_column(ForeignKey("locale_variants.id"), nullable=False)
    content_type: Mapped[str] = mapped_column(String(32), nullable=False)
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="draft")
    ...
    __table_args__ = (
        ...
        CheckConstraint("content_type in ('journal','artwork')", name="ck_content_items_type"),
    )
