...
def upgrade() -> None:
    ...
    op.execute(sa.text("CREATE FUNCTION prevent_locked_evidence_set_mutation() RETURNS trigger AS $$ BEGIN IF OLD.locked_at IS NOT NULL THEN RAISE EXCEPTION 'locked_evidence_set_is_immutable'; END IF; RETURN OLD; END; $$ LANGUAGE plpgsql"))
