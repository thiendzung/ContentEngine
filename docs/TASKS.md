...
- [ ] neither Writer input contains or depends on the sibling draft;
- [ ] one immutable/versioned `journal_draft` Artifact per locale;
...
- [ ] exact locale retry reuses the same draft artifact without another model call;
...
- [ ] immutable run SettingsSnapshot remains unchanged;
- [ ] both Writers use exactly `codex_cli / gpt-5.6-luna` via the already-approved `angle` route;
- [ ] real `vi-VN` Writer execution creates one draft;
- [ ] real `en` Writer execution creates one independent draft from the same Outline;
- [ ] each run returns to `waiting_approval`;
- [ ] `unresolved_factual_claim_count = 0` for both drafts;
- [ ] exact repeated executions prove zero extra model call/artifact;
...
→ Founder merge
→ Agent Local real vi-VN + en drafts
